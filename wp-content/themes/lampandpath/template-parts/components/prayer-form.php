...
$lampandpath_note  = lampandpath_home_option( 'prayer', 'form_note' );
$lampandpath_field = 'mt-2 rounded-xl border-[1.5px] border-field bg-white px-4 text-[17px] text-ink';
?>
<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-lp-prayer-form class="flex flex-col rounded-[20px] border border-line bg-white p-6 sm:p-8">
	...
	<button type="submit" class="mt-6 h-13 rounded-full bg-accent text-[17px] leading-none font-semibold text-white hover:bg-accent-strong"><?php esc_html_e( 'Send prayer request', 'lampandpath' ); ?></button>
